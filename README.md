...
В системе есть две пользовательские роли:
...
- База данных: PostgreSQL, с локальным fallback на SQLite
...
Если `POSTGRES_DB` не задан, проект использует SQLite-файл `backend/db.sqlite3`.
...
Команда запрашивает только `username` и пароль.
...
- `POST /api/auth/login/`
- `POST /api/auth/register/` — endpoint оставлен для совместимости, но не используется UI
...
- Email не используется ни при логине, ни при создании пользователей.
- Фронтенд не рассчитывает итоговые метрики сам.
- Поле `criticality_level` скрыто в публичном `play`-endpoint.
- Таймер стресс-фактора показывается только для тестов, где включен `SRI`.
- В кабинете HR можно фильтровать таблицу тестов и статистику по профилю оценки.
- Архивные профили нельзя выбрать для новых тестов, но старые тесты по ним остаются видимыми.
- В интерфейсе есть переключатель светлого и тёмного режима.
